...
namespace BlueBird_Play
{
    class AttackReplyTweet : ITask
    {
        ...
        int life = 1;
        string target = "";
        public AttackReplyTweet(ITask.HALDLE_INFO character, string target_id, string tweet_text, List<string> tag_list, int task_life)
        {
        ...
        public bool RunTask(ICharacter character)
        {
            ...
            if (json.ContainsKey("id_str"))
            {
                ...
                if (life > 0)
                {
                    AttackReplyTweet reply = new AttackReplyTweet(new ITask.HALDLE_INFO { type = ITask.HALDLE_TYPE.RANDOM_MUL_WITHOUT, count = life - random.Next(0, life), without_id = new string[] { character.character_id } }, json["id_str"].ToString(), Bullet.GetBullet(text), tag, life - random.Next(0, life));
